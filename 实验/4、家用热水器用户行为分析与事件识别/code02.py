...
# 读取数据
data = pd.read_excel('water_heater.xls')  # 读取热水器使用数据记录
sj = pd.read_excel('sj.xls')  # 读取用水事件记录
# 转换时间格式
...
sj_final = sj[['事件序号', '总用水时长', '停顿次数', '总用水量', '平均水流量']]
sj_final.to_excel('sj_final.xls', index=False)
